<script>
    ...
    // ... (dispatch, keyMap, colors are unchanged) ...
    ...
    // ... (keypadLayout, gamepadMap, pressedKeys are unchanged) ...
    const keypadLayout = [
    ...
    // MODIFIED: SCALE is now a reactive variable based on settings.
        ...
        for (let y = 0; y < DISPLAY_HEIGHT; y++) {
            for (let x = 0; x < DISPLAY_WIDTH; x++) {
                if (displayBuffer[y * DISPLAY_WIDTH + x]) {
                    // MODIFIED: Use reactive scale variable
    ...
    // MODIFIED: The reactive `scale` variable will trigger this when it changes.
    $: if (canvasElement) {
...
<!-- ... (Gamepad component is unchanged) ... -->
...
<div class="flex flex-col md:flex-row h-full p-3 space-y-3 md:space-y-0 md:space-x-3">
    <section class="flex-grow flex items-center justify-center bg-gray-900 rounded-md shadow-inner p-3">
        <!-- MODIFIED: Canvas dimensions are now reactive -->
        <canvas
