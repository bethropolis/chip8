<script>
    ...
    /**
     * The CHIP-8 key currently being remapped, or null if not remapping.
     * @type {number|null}
     */
    let remappingKey = null;

    /**
     * Array of keybinding objects: { chip8Key: number, keyboardKey: string }
     * @type {Array<{chip8Key: number, keyboardKey: string}>}
     */
    let keybindings = [];
    ...
    /**
     * Save the current settings and close the modal.
    ...
    /**
     * Begin remapping a CHIP-8 key to a new keyboard key.
     * @param {Event} event
     * @param {number} chip8KeyToRemap
     */
    ...
    /**
     * Handle the keydown event for remapping a key.
    ...
    /**
     * End remapping for a CHIP-8 key, restoring the original key if needed.
     * @param {Event} event
     * @param {number} chip8Key
     */
    ...
    /**
     * Open a dialog to select the ROMs directory and update the setting.
     * @returns {Promise<void>}
     */
